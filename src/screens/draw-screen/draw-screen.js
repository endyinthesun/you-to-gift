...
import {TopDrawItem, Notification} from '_molecules/index';
...
export default function DrawScreen({route, navigation}) {
  ...
  const {id, imageUrl, amountSubs, formatBudget, dateEnd, timeEnd} =
    route.params;
  ...
  useEffect(() => {
    _draw(id).then(({data}) => {
      ...
      setItemData({
        ...
        currency: data.valute.symbol,
        type: data.get_type,
        userName: data.chanel.title,
      });
      setOnLoad(true);
    });
  }, []);
      ...
      <View style={{flex: 1, overflow: 'hidden'}}>
        ...
        <View style={{flex: 1}}>
            ...
            {onLoad ? (
              <View style={contentContainerStyles.container}>
                  ...
                  <View style={styles.details}>
                    ...
                    <View style={styles.timeInfo}>
                      <View style={styles.detailsTime}>
                        <CalendarIcon />
                        <Text style={styles.detailsTimeText}>{dateEnd}</Text>
                      </View>
                      <View style={styles.detailsTime}>
                        <ClockIcon />
                        <Text style={styles.detailsTimeText}>{timeEnd}</Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.instPost}>
                      <InstPost
                        postImgUrl={itemData.postUrl}
                        userName={itemData.userName}
                        userAvaImgUrl={imageUrl}
                      />
                    </View>
                  </View>
                </View>
                ...
                <View style={styles.info}>
                  ...
                  <View style={styles.secondaryBtn}>
                    <SecondaryBtn
                      icon={<ExcelIcon style={styles.iconExcel} />}
          ...
          <Modal
            ...
            <ModalInfoScreen onPress={() => setModalInfoVisible(false)} />
          </Modal>
        </View>
      </View>
    </SafeAreaView>
